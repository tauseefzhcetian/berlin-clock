...
public class TimeConverterImpl implements TimeConverter {
	...
	public String convertTime(String aTime) {
		...
		result.append((seconds % 2 == 0)?"Y":"O");
		result.append("\r\n");
		
		// second row to display 5 hours
		for (int i = 0; i < 4; i++) {
			result.append((i < noOf5Hours)?"R":"O");
		}
		result.append("\r\n");
		
		// third row to display hours
		for (int i = 0; i < 4; i++) {
			result.append((i < noOfHours)?"R":"O");
		}
		result.append("\r\n");
		
		// fourth row to display 5 minutes
		for (int i = 0; i < 11; i++) {
			result.append((i < noOf5Minutes)?(i == 2 || i == 5 || i == 8)?"R":"Y":"O");
		}
		result.append("\r\n");
		
		// fifth row to display minutes
		for (int i = 0; i < 4; i++) {
			result.append((i < noOfMinutes)?"Y":"O");
		}
